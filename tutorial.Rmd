...
**`isolate` should be used inside a reactive environment such as `render*` functions.** For example, I wanted to isolate one variable which was used many times but I didn't want to use `isolate` in every line where the variable is used. I was placing `isolate` outside `renderPlot` and then using it inside the reactive environment but was getting weird errors.
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      fluidRow(column(6, uiOutput("xvar")), column(6, uiOutput("yvar"))),
      actionButton("mk_pl", label = "Make plot")),
...
server <- function(input, output, session) {
  ...
  output$scatter <- renderPlot({
    # Make button dependency
    input$mk_pl
    
    # isolate should go INSIDE a reactive environment
    variables <-
      isolate({
        list(iris[input$xvar],
             iris[input$yvar])
      })
    ...
    plot(variables[[1]][, 1],
         variables[[2]][, 1],
         xlab = names(variables[[1]]),
         ylab = names(variables[[2]]))
  })
}
...
Why? It makes sense. In a reactive environment where everything **will** be updated when you change something, you isolate some expression not to be recomputed.
